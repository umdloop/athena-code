...
double SERVOHardwareInterface::calculate_joint_position_from_motor_position(
  double motor_position, int gear_ratio)
{
  return (motor_position * (M_PI / 180.0)) / gear_ratio;
}

double SERVOHardwareInterface::calculate_joint_displacement_from_motor_position(
  double motor_position, int gear_ratio, double meters_per_deg)
{
  return (motor_position * meters_per_deg) / gear_ratio;
}

double SERVOHardwareInterface::calculate_joint_angular_velocity_from_motor_velocity(
  double motor_velocity, int gear_ratio)
{
  return (motor_velocity * (M_PI / 180.0)) / gear_ratio;
}

double SERVOHardwareInterface::calculate_joint_linear_velocity_from_motor_velocity(
  double motor_velocity, int gear_ratio, double meters_per_deg)
{
  return (motor_velocity * meters_per_deg) / gear_ratio;
}

int16_t SERVOHardwareInterface::calculate_motor_position_from_desired_joint_position(
  double joint_position, int gear_ratio)
{
  return static_cast<int16_t>(std::round((joint_position * (180.0 / M_PI)) * gear_ratio));
}

int16_t SERVOHardwareInterface::calculate_motor_position_from_desired_joint_displacement(
  double joint_position, int gear_ratio, double meters_per_deg)
{
  return static_cast<int16_t>(std::round((joint_position / meters_per_deg) * gear_ratio));
}

int16_t SERVOHardwareInterface::calculate_motor_velocity_from_desired_joint_angular_velocity(
  double joint_velocity, int gear_ratio)
{
  return static_cast<int16_t>(std::round((joint_velocity * (180.0 / M_PI)) * gear_ratio));
}

int16_t SERVOHardwareInterface::calculate_motor_velocity_from_desired_joint_linear_velocity(
  double joint_velocity, int gear_ratio, double meters_per_deg)
{
...
void SERVOHardwareInterface::logger_function()
{
  ...
  for (const auto & joint : SERVOJoints_) {
  ...
  RCLCPP_INFO(rclcpp::get_logger("SERVOHardwareInterface"), "%s", oss.str().c_str());
}

void SERVOHardwareInterface::format_control_command(CANLib::CanFrame & frame, ServoJoint & joint)
{
  std::fill(std::begin(frame.data), std::end(frame.data), 0x00);

  if (
    joint.control_level == integration_level_t::POSITION &&
    joint.servo_type == servo_type_t::STANDARD &&
    ...
    joint.joint_command_position != joint.prev_joint_command_position)
  {
    ...
    if (joint.joint_type == joint_type_t::REVOLUTE) {
      joint_angle = calculate_motor_position_from_desired_joint_position(
        joint.joint_command_position, joint.gear_ratio);
    } else {
      joint_angle = calculate_motor_position_from_desired_joint_displacement(
        joint.joint_command_position, joint.gear_ratio, joint.meters_per_deg);
    }
    ...
    joint.joint_command_velocity != joint.prev_joint_command_velocity)
  {
    ...
    if (joint.joint_type == joint_type_t::REVOLUTE) {
      joint_velocity = calculate_motor_velocity_from_desired_joint_angular_velocity(
        joint.joint_command_velocity, joint.gear_ratio);
    } else {
      joint_velocity = calculate_motor_velocity_from_desired_joint_linear_velocity(
        joint.joint_command_velocity, joint.gear_ratio, joint.meters_per_deg);
    }
...
bool SERVOHardwareInterface::format_maintenance_command(
  ...
  const hardware_interface::HardwareInfo & info)
{
  ...
  for (const auto & joint : info_.joints) {
    const std::string servo_type_param = joint.parameters.at("servo_type");
    const std::string joint_type_param = joint.parameters.at("joint_type");

    servo_type_t servo_type = servo_type_t::STANDARD;
    if (servo_type_param == "continuous") {
      servo_type = servo_type_t::CONTINUOUS;
    }

    joint_type_t joint_type = joint_type_t::REVOLUTE;
    if (joint_type_param == "prismatic") {
      joint_type = joint_type_t::PRISMATIC;
    }

    const double meters_per_deg = joint.parameters.count("meters_per_deg") ?
      std::abs(std::stod(joint.parameters.at("meters_per_deg"))) : std::numeric_limits<double>::quiet_NaN();

    double rated_max = 0.0;
    if (servo_type == servo_type_t::STANDARD) {
      rated_max = std::abs(std::stod(joint.parameters.at("max_pos")));
      rated_max = joint_type == joint_type_t::REVOLUTE ?
        rated_max * (M_PI / 180.0) : rated_max * meters_per_deg;
    } else {
      rated_max = std::abs(std::stod(joint.parameters.at("max_vel")));
      rated_max = joint_type == joint_type_t::REVOLUTE ?
        rated_max * (M_PI / 180.0) : rated_max * meters_per_deg;
    }
    ...
    const integration_level_t initial_mode =
      servo_type == servo_type_t::CONTINUOUS ? integration_level_t::VELOCITY : integration_level_t::POSITION;

    SERVOJoints_.push_back(ServoJoint{
      joint.name,
  ...
  num_joints = static_cast<int>(SERVOJoints_.size());
...
std::vector<hardware_interface::CommandInterface> SERVOHardwareInterface::export_command_interfaces()
{
  ...
  const rclcpp_lifecycle::State &)
{
  if (!canBus.open(
      can_interface,
      std::bind(&SERVOHardwareInterface::on_can_message, this, std::placeholders::_1)))
  {
    RCLCPP_ERROR(rclcpp::get_logger("SERVOHardwareInterface"), "Failed to open CAN interface");
    return hardware_interface::CallbackReturn::ERROR;
  }
  return hardware_interface::CallbackReturn::SUCCESS;
}
...
void SERVOHardwareInterface::on_can_message(const CANLib::CanFrame & frame)
{
  ...
  for (auto & joint : SERVOJoints_) {
    ...
    if (command_nibble == (static_cast<uint8_t>(StatusCommands::MOTOR_STATE) >> 4) ||
      ...
      if (joint.joint_type == joint_type_t::REVOLUTE) {
        joint.motor_position = calculate_joint_position_from_motor_position(
          raw_position, joint.gear_ratio);
        joint.motor_velocity = calculate_joint_angular_velocity_from_motor_velocity(
          raw_velocity, joint.gear_ratio);
      } else {
        joint.motor_position = calculate_joint_displacement_from_motor_position(
          raw_position, joint.gear_ratio, joint.meters_per_deg);
        joint.motor_velocity = calculate_joint_linear_velocity_from_motor_velocity(
          raw_velocity, joint.gear_ratio, joint.meters_per_deg);
      }
      return;
    }
...
hardware_interface::return_type SERVOHardwareInterface::write(
  const rclcpp::Time &, const rclcpp::Duration & period)
{
  ...
  if (update_rate > 0 && elapsed_update_time > (1.0 / static_cast<double>(update_rate))) {
    elapsed_update_time = 0.0;
    if (!SERVOJoints_.empty()) {
      auto & joint = SERVOJoints_[static_cast<size_t>(write_count % num_joints)];
      ++write_count;
      CANLib::CanFrame frame;
      frame.id = can_command_id;
  ...
  const std::vector<std::string> & stop_interfaces)
{
  ...
  for (const auto & ifname : stop_interfaces) {
    for (size_t i = 0; i < SERVOJoints_.size(); ++i) {
      const auto & joint = SERVOJoints_[i];
